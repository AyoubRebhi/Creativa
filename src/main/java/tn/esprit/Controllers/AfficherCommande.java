...
public class AfficherCommande implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        searchBarEvents.setOnKeyReleased(event -> {
            try {
                String searchText = searchBarEvents.getText();
                List<Commande> searchResults = SR.searchCommand(searchText); // Assuming there's a method in ServiceCommande for searching commands
                updateCommandeCards(searchResults);
            } catch (SQLException e) {
    ...
    public void updateCommandeCards(List<Commande> searchResults) {
        EMPHBox.getChildren().clear(); // Clear existing command cards
        for (Commande result : searchResults) {
            VBox card = createCardView(result);
            EMPHBox.getChildren().add(card);
        }
    }
