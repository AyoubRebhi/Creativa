...
private int id;
  public int  nouveauxPoints;


    @FXML
    void AjouterCommande(ActionEvent event) throws SQLException {
        // Vérifier si tous les champs obligatoires sont remplis
        if (code_promoTF.getText().isEmpty()  || MTtotalLabel.getText().isEmpty()) {
        ...
        c.setId_user(session.id_utilisateur);
        ...
        s1.AJOUTERPoints(session.id_utilisateur,nouveauxPoints);
...
// Définir la date estimée de livraison dans le DatePicker
    ...
    CarteFideliteService s1=new CarteFideliteService();


    public void setProjet(int id, Projet projet) {
        this.projet=projet;
        this.id = id;
        String titre = projet.getTitre();

        id_userTF.setText(projet.getTitre());
        try {
            prixProduitLabel.setText(String.valueOf(projet.getPrix()));



            // Calculer le montant de la remise
            float prixFloat = Float.parseFloat(prixProduitLabel.getText());


            // Calculer le prix après remise
            float prixApresRemise = prixFloat +8;

            // Mettre à jour les labels avec les nouveaux prix
            MTtotalLabel.setText(String.valueOf(prixApresRemise));

            if(s1.carteExistsForUser(session.id_utilisateur)!=-1){
                 nouveauxPoints = (int) (projet.getPrix() / 10);
label1.setVisible(true);
label2.setText(String.valueOf(nouveauxPoints)+"points");
label2.setVisible(true);
    ...
    public void ajouterPoints(float montantAchat) {
        // Supposons que vous attribuez 1 point pour chaque tranche de 10 unités de la monnaie locale
        int nouveauxPoints = (int) (montantAchat / 10);
//        points += nouveauxPoints;
        System.out.println(nouveauxPoints + " points ajoutés à la carte de fidélité.");
    }
