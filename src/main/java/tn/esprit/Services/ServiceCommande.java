...
import tn.esprit.Interfaces.InterfaceCRUD1;
import tn.esprit.Models.Commande;
import tn.esprit.Models.SessionCommande;
import tn.esprit.Utils.MaConnexion;

import java.util.*;
import java.sql.Connection;

public class ServiceCommande implements InterfaceCRUD1<Commande> {
    ...
    public void ajouter(Commande c) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(req, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (generatedKeys.next()) {
                ...
                System.out.println("Commande ajoutée avec succès! ID de la commande : " + idCommande);

                // Mettre à jour la session avec l'ID de la commande
                SessionCommande.id_commande= idCommande;
            } else {
    ...
    public int getIdUtilisateurParNomComplet(String nomComplet) throws SQLException {
        ...
        String req = "SELECT id_user FROM user WHERE CONCAT(first_name, ' ', last_name) = ?";
        try (PreparedStatement ps = conn.prepareStatement(req)) {
